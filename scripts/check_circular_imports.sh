...
echo "测试 2: config_manager + deepseek_strategy 导入"
python3 -c "
import sys
sys.path.insert(0, '$PROJECT_ROOT')
from utils.config_manager import ConfigManager
from strategy.deepseek_strategy import DeepSeekAIStrategy
print('✅ config_manager + deepseek_strategy 导入成功')
"
echo ""
...
print('✅ 完整导入链验证成功')
print('   config_manager → deepseek_client → multi_agent_analyzer → trading_logic → deepseek_strategy')
"
echo ""
