...
def main():
    ...
    if oi_response.get("status_code") == 200 and oi_response.get("data"):
        ...
        fields = [
            ...
            ("value", oi_data.get("value"), "✅ 是", "OI 数量 (BTC)"),
    ...
    if liq_response.get("status_code") == 200 and liq_response.get("data"):
        ...
        if history:
            ...
            history_fields = [
                ...
                ("l", sample.get("l"), "✅ 是", "多头爆仓 (BTC)"),
                ("s", sample.get("s"), "✅ 是", "空头爆仓 (BTC)"),
            ]
...
│ API 端点              │ 使用的字段           │ 用途                     │
├───────────────────────┼─────────────────────┼─────────────────────────┤
│ /open-interest        │ value               │ 持仓量 (BTC → USD)       │
│ /funding-rate         │ value               │ 资金费率 (%)             │
│ /liquidation-history  │ history[].l, .s     │ 多空爆仓量 (BTC → USD)   │
└─────────────────────────────────────────────────────────────────────────┘
