...
async def create_demo_signal_data():
    """Create demo signal data for testing"""
    try:
        signals = create_demo_signals()
        return {"success": True, "message": f"Created {len(signals)} demo signals"}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def create_demo_notification_data():
    """Create demo notification data for testing"""
    try:
        notifications = create_demo_notifications()
        return {"success": True, "message": f"Created {len(notifications)} demo notifications"}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
