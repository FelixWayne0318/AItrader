...
export function AdminTradeAnalysis() {
  ...
  const { summary, isLoading: summaryLoading } = useAdminSummary(days);
  const { trades, isLoading: tradesLoading } = useFullTrades(100);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <h2 className="text-2xl font-bold">交易质量深度分析</h2>
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="text-right">盈利数</TableHead>
                <TableHead className="text-right">胜率</TableHead>
                <TableHead className="text-right">平均 R/R</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {["HIGH", "MEDIUM", "LOW"].map((level) => {
                const stats = summary.confidence_accuracy[level as keyof typeof summary.confidence_accuracy];
                const avgRR = trades
                  .filter(t => t.confidence === level && t.direction_correct)
                  .reduce((acc, t) => acc + t.actual_rr, 0) /
                  (trades.filter(t => t.confidence === level && t.direction_correct).length || 1);
      ...
      <Card>
        <CardHeader>
          <CardTitle>出场方式分布</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="space-y-4">
            {Object.entries(summary.exit_type_distribution).map(([type, count]) => {
              const percent = (count / summary.total_evaluated) * 100;
              return (
                <div key={type} className="space-y-1">
                  <div className="flex items-center justify-between text-sm">
                    <span>{type}</span>
                    <span className="font-mono">{count} ({percent.toFixed(1)}%)</span>
                  </div>
        ...
        <CardContent>
          <div className="space-y-2">
            {[
              ...
              { label: "负数", min: -Infinity, max: 0.0 },
            ].map(({ label, min, max }) => {
              const count = trades.filter(t => t.actual_rr >= min && t.actual_rr < max).length;
              const percent = (count / trades.length) * 100;
              return (
      ...
      <Card>
        <CardHeader>
          <CardTitle>完整交易记录 (最近 100 笔)</CardTitle>
        </CardHeader>
        <CardContent>
            ...
            <Table>
              <TableHeader className="sticky top-0 bg-background">
                <TableRow>
                  ...
                  <TableHead className="text-right">入场</TableHead>
                  <TableHead className="text-right">出场</TableHead>
                ...
                {trades.slice(0, 100).map((trade, idx) => (
                  <TableRow key={idx}>
                    <TableCell className="font-mono text-xs">
                      {new Date(trade.timestamp).toLocaleDateString('zh-CN', { month: 'numeric', day: 'numeric', hour: '2-digit', minute: '2-digit' })}
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell className="text-right font-mono text-xs">
                      {trade.planned_rr.toFixed(1)} → {trade.actual_rr.toFixed(1)}
                    </TableCell>
