...
def _volume_weight_factor(bar_volume: float, all_volumes: List[float]) -> float:
    ...
    - Percentile naturally normalizes across 1D/4H/15M
...
def detect_swing_points(
    ...
    timeframe : str
        Timeframe label for same-source weight capping ("1d", "4h", "15m").
    base_weight : float
        Base weight for candidates (1D=2.0, 4H=1.5, 15M=0.8).
    level : str
        SRLevel for candidates (1D=MAJOR, 4H=INTERMEDIATE, 15M=MINOR).
    left_bars : int
