...
class DeepSeekAIStrategyConfig(StrategyConfig, frozen=True):
    ...
    telegram_notify_sltp_update: bool = True  # v5.0: SL/TP 动态更新通知 (原 trailing_stop)
...
class DeepSeekAIStrategy(Strategy):
    ...
    def __init__(self, config: DeepSeekAIStrategyConfig):
        ...
        # Track SL/TP state for each position (unified: S/R dynamic + profit-lock)
        ...
        #   "instrument_id": {
        #       "entry_price": float,
        #       "highest_price": float (session high — PnL display),
        #       "lowest_price": float (session low — PnL display),
    ...
    def _recover_sltp_on_start(self):
        ...
        try:
            ...
            if has_sl:
                ...
                # Restore sltp_state from existing orders
                instrument_key = str(self.instrument_id)
                if instrument_key not in self.sltp_state:
                    for o in reduce_only_orders:
                        if 'STOP' in str(o.order_type):
                            ...
                            self.log.info(f"📌 Restored sltp_state from Binance SL @ ${sl_price:,.0f}")
                            break
                return
    ...
    def on_timer(self, event):
        ...
        try:
            ...
            # v5.0: Unified SL/TP reevaluation (S/R dynamic + profit-lock)
            # Replaces separate trailing stop + S/R reevaluation + legacy dynamic update
        ...
        try:
            ...
            try:
                ...
                if pos_data and pos_data.get('quantity', 0) != 0:
                    ...
                    # v4.2: Get SL/TP from sltp_state
                    instrument_key = str(self.instrument_id)
    ...
    def _validate_sltp_for_entry(
        ...
        if is_valid:
            ...
            # v5.0: Level 2 — S/R-based SL/TP with ATR buffer (only path)
            if hasattr(self, 'latest_sr_zones_data') and self.latest_sr_zones_data:
                try:
    ...
    def _replace_sltp_orders(
        ...
        try:
            ...
            try:
                ...
                instrument_key = str(self.instrument_id)
                if instrument_key in self.sltp_state:
                    self.sltp_state[instrument_key]["sl_order_id"] = str(new_sl_order.client_order_id)
                    self.sltp_state[instrument_key]["current_sl_price"] = new_sl_price
                    self.sltp_state[instrument_key]["quantity"] = new_total_quantity
            except Exception as e:
            ...
            try:
                ...
                instrument_key = str(self.instrument_id)
                if instrument_key in self.sltp_state:
                    self.sltp_state[instrument_key]["current_tp_price"] = new_tp_price
            except Exception as e:
    ...
    def _reevaluate_sltp_for_existing_position(self):
        """
        v5.0: S/R-based dynamic SL/TP reevaluation for existing positions.

        Called every on_timer cycle. Uses calculate_sr_based_sltp() which
        already includes ATR-adaptive buffer distance.

        Logic:
        1. Recalculate SL/TP via S/R zones + ATR buffer
        2. SL only moves in favorable direction (LONG: up, SHORT: down)
        3. If S/R fails → keep existing SL/TP (position remains protected)
        4. Skip if change < dynamic_update_threshold_pct
        """
        try:
            current_position = self._get_current_position_data()
            if not current_position:
            ...
            # Track highest/lowest for PnL display
            cur_high = state.get("highest_price", current_price)
            cur_low = state.get("lowest_price", current_price)
            if current_price > cur_high:
                state["highest_price"] = current_price
            if current_price < cur_low:
                state["lowest_price"] = current_price

            # --- S/R-based recalculation (ATR buffer included) ---
            if not hasattr(self, 'latest_sr_zones_data') or not self.latest_sr_zones_data:
            ...
            if not new_sl or not new_tp or new_sl <= 0 or new_tp <= 0:
                # S/R rejected — keep existing SL/TP, warn
                self.log.warning(
                ...
                # Send Telegram alert (rate-limit: 15 min cooldown)
                if self.telegram_bot and self.enable_telegram:
                    try:
                        import time
            ...
            final_tp = new_tp  # TP can move both ways (R/R already validated by calculator)

            # Validate SL won't immediately trigger (prevent Binance -2021)
            if position_side == 'long' and final_sl >= current_price:
                self.log.debug("📊 SL/TP reevaluation: SL >= current price for LONG, skipping")
                return
            if position_side == 'short' and final_sl <= current_price:
                self.log.debug("📊 SL/TP reevaluation: SL <= current price for SHORT, skipping")
                return
            ...
            tp_changed = (old_tp and old_tp > 0 and final_tp and final_tp > 0 and
                          abs(final_tp - old_tp) / old_tp > threshold)

            if not sl_changed and not tp_changed:
                self.log.debug("📊 SL/TP reevaluation: no significant change, skipping")
                return
            ...
            self._replace_sltp_orders(
                ...
                position_side=position_side,
                new_sl_price=final_sl,
                new_tp_price=final_tp if final_tp and final_tp > 0 else None,
            )
            ...
            try:
                if self.telegram_bot:
                    update_msg = self.telegram_bot.format_dynamic_sltp_update({
                        ...
                        'new_tp': final_tp if final_tp and final_tp > 0 else 0,
                        'sl_changed': sl_changed,
                        'tp_changed': bool(tp_changed),
                    })
                    self.telegram_bot.send_message_sync(update_msg)
            except Exception as tg_err:
                self.log.debug(f"Failed to send SL/TP Telegram notification: {tg_err}")

        except Exception as e:
            self.log.warning(f"⚠️ SL/TP reevaluation failed (position still protected): {e}")
    ...
    def on_position_opened(self, event):
        ...
        # Update sltp_state with actual entry price
        if instrument_key in self.sltp_state:
            self.sltp_state[instrument_key]["entry_price"] = entry_price
            self.sltp_state[instrument_key]["highest_price"] = entry_price
            self.sltp_state[instrument_key]["lowest_price"] = entry_price

            self.log.debug(
                f"📊 Updated sltp_state with actual entry price: ${entry_price:,.2f}"
            )
        else:
            self.sltp_state[instrument_key] = {
                "entry_price": entry_price,
        ...
        if hasattr(self, '_pending_sltp') and self._pending_sltp:
            ...
            try:
                ...
                self.sltp_state[instrument_key]["sl_order_id"] = str(sl_order.client_order_id)
                self.sltp_state[instrument_key]["current_sl_price"] = sl_price
            ...
            try:
                ...
                self.sltp_state[instrument_key]["current_tp_price"] = tp_price
    ...
    def on_position_closed(self, event):
        ...
        if self.telegram_bot and self.enable_telegram and self.telegram_notify_positions:
            try:
                ...
                # Get SL/TP from sltp_state (stored before clearing)
    ...
    def on_position_changed(self, event):
        ...
        if instrument_key in self.sltp_state:
            ...
            old_qty = self.sltp_state[instrument_key].get("quantity", 0)
    ...
    def _cmd_risk(self) -> Dict[str, Any]:
        ...
        try:
            ...
            msg += f"• S/R 动态 SL/TP: {'✅' if self.dynamic_sltp_update_enabled else '❌'}\n"
    ...
    def _cmd_config(self) -> Dict[str, Any]:
        ...
        try:
            ...
            msg += f"  S/R 动态更新: {'✅' if self.dynamic_sltp_update_enabled else '❌'}\n"
