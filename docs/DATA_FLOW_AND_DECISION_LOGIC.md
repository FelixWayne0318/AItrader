...
> 版本: v3.2 | 更新日期: 2026-01-30
>
> **v3.2 更新**: 移除所有本地硬编码规则，完全符合 TradingAgents 设计
> **v3.1 更新**: 移除趋势方向权限检查
> **v3.0 重大更新**: AI 完全自主决策架构
...
- **本地职责**: 仅收集原始数据，无任何硬编码规则
- **AI 职责**: 完全自主分析数据、判断趋势、决定是否参考支撑阻力位
- **执行层**: 无本地风控，AI 决策即最终决策
...
│     AI 看到: "Support: $83,805" 由 AI 自己判断是否参考                      │
└─────────────────────────────────────────────────────────────────────────────┘
                                      │
                                      ▼
┌─────────────────────────────────────────────────────────────────────────────┐
...
│  └──────────────────────────────────────────────────────────────────────┘  │
│                                                                              │
│  ⚠️ v3.2: AI 完全自主决策，无任何本地硬编码规则                             │
│     Judge 自主评估证据强度，做出 LONG/SHORT/HOLD 决策                       │
│     AI 看到 support/resistance 数据，自己判断是否在该位置交易              │
│                                                                              │
...
└─────────────────────────────────────────────────────────────────────────────┘
                                      ...
                                      ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│  PHASE 3: 无本地风控 (v3.2 - AI 决策即最终决策)                              │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │ ✅ AI Signal = Final Signal (无本地过滤)                            │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                                                                              │
│  ⚠️ v3.2 移除的所有本地判断:                                                │
│     ❌ 趋势方向权限检查 (allow_long/allow_short)                            │
│     ❌ 支撑/阻力位边界检查 (proximity_threshold)                            │
│     ❌ 仓位乘数调整 (multiplier)                                            │
│     ❌ 决策层方向匹配检查 (ALLOW_LONG/ALLOW_SHORT)                          │
│     ❌ RSI 入场范围 [35, 65]                                                │
│     ❌ 确认计数框架 (bullish_count/bearish_count)                           │
│                                                                              │
│  参考: TradingAgents (UCLA/MIT) 不计算支撑阻力位，由 AI 从 SMA/BB 推断     │
